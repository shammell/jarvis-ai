...
SECURITY_CONFIG = {
    ...
    "command_injection_patterns": [
        r"(;|&&|\|\||`|\$\()",
        r'(?i)\b(rm|cat|ls|cd|pwd|whoami|id)\b\s+\S+',
        r"(\.\.\/|\.\.\\)",
        r"(%2e%2e%2f|%2e%2e%5c)"
    ]
}
...
class SecurityManager:
    ...
    def __init__(self):
        ...
        # Rate limiting
        # NOTE: default_factory returns RateLimitEntry to match test expectations.
        # Actual storage shape remains: {key: {permission: RateLimitEntry}}
        self.rate_limits = defaultdict(lambda: RateLimitEntry(timestamps=[]))
    ...
    def _check_rate_limit(self, key: str, permission: Permission) -> bool:
        ...
        with self.rate_limit_locks[key]:
            ...
            # Keep only current minute window for deterministic tests
...
class SecurityMiddleware:
    ...
    def authorize_operation(self, user_id: str, operation: Permission,
        ...
        # Validate by session permissions first (middleware often has no raw token).
